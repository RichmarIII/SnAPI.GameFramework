...
struct UiViewportTransform
{
    ...
    [[nodiscard]] bool IsValid() const
    {
        return std::isfinite(OutputX) && std::isfinite(OutputY) && std::isfinite(OutputWidth) &&
               ...
               std::isfinite(InputScaleX) && std::isfinite(InputScaleY) &&
               OutputWidth > 0.0f && OutputHeight > 0.0f && UiWidth > 0.0f && UiHeight > 0.0f &&
               InputScaleX > 0.0f && InputScaleY > 0.0f;
    }
    ...
    [[nodiscard]] std::pair<float, float> MapWindowPointToUi(const float WindowX, const float WindowY) const
    {
        ...
        const float LocalX = WindowX - OutputX;
        const float LocalY = WindowY - OutputY;
        const float ScaleX = (UiWidth / OutputWidth) * InputScaleX;
        const float ScaleY = (UiHeight / OutputHeight) * InputScaleY;
...
bool GameRuntime::ProcessPlatformAndUiInput()
{
    ...
    if (InputInitialized)
    {
        ...
        if (ForwardToUi)
        {
            ...
            {
                auto& UiSystemForTransform = m_world->UI();
                ...
                if (const auto* RootContext = UiSystemForTransform.Context(UiSystemForTransform.RootContextId()))
                {
                    const float DpiScale = RootContext->GetDpiScale();
                    if (std::isfinite(DpiScale) && DpiScale > 0.0f)
                    {
                        UiTransform.InputScaleX = DpiScale;
                        UiTransform.InputScaleY = DpiScale;
                    }
                }
            ...
            for (const auto& Event : *Events)
            {
                ...
                case SnAPI::Input::EInputEventType::MouseMove:
                    if (const auto* MoveData = std::get_if<SnAPI::Input::MouseMoveEvent>(&Event.Data))
                    {
                        ...
                        if (LogInputEvents)
                        {
                            const auto MappedPointer = UiSystem.MapInputPointToScreenPoint(UiPointer.Position);
                            std::cerr << "[GameFramework][UI] pointer_move raw_x=" << MoveData->X
                                      << " raw_y=" << MoveData->Y
                                      << " ui_x=" << UiPointer.Position.X
                                      << " ui_y=" << UiPointer.Position.Y
                                      << " mapped_x=" << MappedPointer.X
                                      << " mapped_y=" << MappedPointer.Y
                                      << " output=" << UiTransform.OutputWidth << 'x' << UiTransform.OutputHeight
                                      << " ui=" << UiTransform.UiWidth << 'x' << UiTransform.UiHeight
                                      << " input_scale=" << UiTransform.InputScaleX << 'x' << UiTransform.InputScaleY
                                      << '\n';
                        }
