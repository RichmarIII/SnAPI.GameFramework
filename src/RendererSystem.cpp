...
public:
    ...
    [[nodiscard]] static bool IsFinite(const float Value)
    {
        return std::isfinite(Value);
    }

    [[nodiscard]] static bool IsPositiveFinite(const float Value)
    {
        return std::isfinite(Value) && Value > 0.0f;
    }

    [[nodiscard]] static bool IsFiniteUiRect(const float X, const float Y, const float W, const float H)
    {
        return IsFinite(X) &&
               IsFinite(Y) &&
               IsPositiveFinite(W) &&
    ...
    [[nodiscard]] static bool IsFiniteUiTextureArea(const float U0, const float V0, const float U1, const float V1)
    {
        return IsFinite(U0) &&
               IsFinite(V0) &&
               IsFinite(U1) &&
    ...
    [[nodiscard]] static bool IsFiniteUiScissor(const float MinX, const float MinY, const float MaxX, const float MaxY)
    {
...
[[nodiscard]] static bool UsesWorldRenderingPreset(const ERenderViewportPassGraphPreset Preset)
{
    switch (Preset)
    {
    case ERenderViewportPassGraphPreset::DefaultWorld:
#if defined(WITH_EDITOR) && WITH_EDITOR
    case ERenderViewportPassGraphPreset::EditorWorld:
#endif
        return true;
    case ERenderViewportPassGraphPreset::None:
    case ERenderViewportPassGraphPreset::UiPresentOnly:
    default:
        return false;
    }
}

[[nodiscard]] static float Halton(const std::uint32_t Index, const std::uint32_t Base)
{
    float Result = 0.0f;
...
[[nodiscard]] static SnAPI::Vector2DF ComputeTaaProjectionJitterNdc(const std::uint64_t FrameIndex, const SnAPI::Size2DU& RenderExtent)
{
    if (RenderExtent.x() == 0u || RenderExtent.y() == 0u)
    {
        return SnAPI::Vector2DF::Zero();
    }

    constexpr std::uint32_t kJitterSequenceLength = 32u;
    const std::uint32_t SampleIndex = static_cast<std::uint32_t>(FrameIndex % kJitterSequenceLength) + 1u;
    const float PixelOffsetX = Halton(SampleIndex, 2u) - 0.5f;
    const float PixelOffsetY = Halton(SampleIndex, 3u) - 0.5f;

    return SnAPI::Vector2DF(
        (2.0f * PixelOffsetX) / static_cast<float>(RenderExtent.x()),
                                     ...
                                     const std::unordered_map<std::uint64_t, float>& ViewportJitterScales)
{
    ...
    std::unordered_map<SnAPI::Graphics::ICamera*, TaaCameraJitterState> CameraStates{};
    std::unordered_set<SnAPI::Graphics::ICamera*> CandidateCameras{};
    const auto ViewportIds = GraphicsApi->RenderViewportIDs();
    auto* GraphicsApiVulkan = static_cast<SnAPI::Graphics::VulkanGraphicsAPI*>(GraphicsApi);
    auto* ActiveCamera = GraphicsApiVulkan ? GraphicsApiVulkan->ActiveCamera() : nullptr;
    if (ActiveCamera)
    {
        CandidateCameras.insert(ActiveCamera);
    }

    for (const auto ViewportID : ViewportIds)
    {
        const auto Config = GraphicsApi->GetRenderViewportConfig(ViewportID);
        ...
        if (Config->pCamera)
        {
            CandidateCameras.insert(Config->pCamera);
        }

        const auto RegisteredPresetIt = RegisteredViewportPassGraphs.find(static_cast<std::uint64_t>(ViewportID));
        const ERenderViewportPassGraphPreset Preset =
            RegisteredPresetIt != RegisteredViewportPassGraphs.end()
                ? RegisteredPresetIt->second
                : ERenderViewportPassGraphPreset::None;
        if (!UsesWorldRenderingPreset(Preset))
        {
            continue;
        }

        auto* Camera = Config->pCamera ? Config->pCamera : ActiveCamera;
        if (!Camera)
        {
            continue;
        }
    ...
    for (auto* Camera : CandidateCameras)
    {
        if (!Camera)
        {
            continue;
...
void RendererSystem::EndFrame()
{
    ...
    if (m_window && m_window->IsOpen())
    {
        ...
        if (BeganFrame)
        {
            ...
            if (m_settings.EnableTaa && m_taaFrameIndex < std::numeric_limits<std::uint64_t>::max())
            {
                ++m_taaFrameIndex;
            }
        }
...
void RendererSystem::FlushQueuedUiPackets()
{
    ...
    {
        if (!RendererSystemOps::IsFiniteUiRect(Entry.X, Entry.Y, Entry.W, Entry.H) ||
            !RendererSystemOps::IsFinite(Entry.GlobalZ) ||
            !RendererSystemOps::IsFinite(Entry.CornerRadius) ||
            !RendererSystemOps::IsFinite(Entry.BorderThickness) ||
            !RendererSystemOps::IsFiniteUiTextureArea(Entry.U0, Entry.V0, Entry.U1, Entry.V1))
        {
            continue;
        }

        if (Entry.HasScissor &&
            !RendererSystemOps::IsFiniteUiScissor(Entry.ScissorMinX, Entry.ScissorMinY, Entry.ScissorMaxX, Entry.ScissorMaxY))
        {
            continue;
        }
                                                             ...
                                                             const bool TrackDefaultPassPointers)
{
    ...
    auto HeightFogPassProperties = PassProperties{
        {AutoGeneratedPass::PropertyNames::PassName.data(), "Height Fog Pass"},
        {FullScreenPass::PropertyNames::MaterialsShadingModel.data(), "PostProcessShadingModel"},
        {FullScreenPass::PropertyNames::MaterialsModule.data(), "HeightFogMaterial"},
        {AutoGeneratedPass::PropertyNames::PassDepthConfig.data(),
         DepthConfig{.WriteDepth = false, .SampleDepth = true, .DepthTest = false, .ReadResourceName = "GBuffer_Depth", .ReadLayout = EImageLayout::DepthStencilReadOnlyOptimal}}};
    if (!m_settings.EnableSsr)
    {
        HeightFogPassProperties[AutoGeneratedPass::PropertyNames::PassInputResourceNameOverrides.data()] =
            ResourceNameMappings{{"Shading_SSR", m_settings.EnableSsgi ? "Shading_SSGI" : "Shading_Out"}};
    }

    auto SSGICompositePassProperties = PassProperties{
        {AutoGeneratedPass::PropertyNames::PassName.data(), "SSGI Composite Pass"},
        {FullScreenPass::PropertyNames::MaterialsShadingModel.data(), "PostProcessShadingModel"},
        {FullScreenPass::PropertyNames::MaterialsModule.data(), "SSGICompositeMaterial"},
    };
    ...
    ResourceNameMappings AtmosphereCompositeInputs{};
    if (m_settings.EnableHeightFog)
    {
        AtmosphereCompositeInputs.emplace_back("Shading_SSR", "HeightFog_Out");
    }
    else if (!m_settings.EnableSsr)
    {
        AtmosphereCompositeInputs.emplace_back("Shading_SSR", m_settings.EnableSsgi ? "Shading_SSGI" : "Shading_Out");
    }
    if (!AtmosphereCompositeInputs.empty())
    {
        AtmosphereCompositePassProperties[AutoGeneratedPass::PropertyNames::PassInputResourceNameOverrides.data()] =
            std::move(AtmosphereCompositeInputs);
    }

    std::string TaaSourceResourceName = "Shading_Out";
    if (m_settings.EnableSsgi)
    {
        TaaSourceResourceName = "Shading_SSGI";
    }
    if (m_settings.EnableSsr)
    {
        TaaSourceResourceName = "Shading_SSR";
    }
    if (m_settings.EnableHeightFog)
    {
        TaaSourceResourceName = "HeightFog_Out";
    }
    if (m_settings.EnableAtmosphere)
    {
        TaaSourceResourceName = "AtmosComposite_Out";
    }

    auto TAAPassProperties = PassProperties{
        {AutoGeneratedPass::PropertyNames::PassName.data(), "TAA Pass"},
        {AutoGeneratedPass::PropertyNames::PassInputResourceNameOverrides.data(),
         ResourceNameMappings{{"TAA_Current", TaaSourceResourceName}}},
    };

    if (m_settings.EnableTaa && !m_settings.EnableBloom)
    {
        ToneMapPassProperties[AutoGeneratedPass::PropertyNames::PassInputResourceNameOverrides.data()] =
            ResourceNameMappings{{"BloomComposite_Out", "TAA_Out"}};
    }
    ...
    ShadowPass::Descriptor MainShadowDescriptor{};
    MainShadowDescriptor.ShadowMapSize = 2048u;
    MainShadowDescriptor.MatchDirectionalLightShadowMapSize = true;

    auto Shadow = std::make_unique<ShadowPass>(MainShadowDescriptor);
    ...
    if (m_settings.EnableSsgi)
    {
        ...
        SSGI->SetIntensity(0.85f);
        SSGI->SetMaxDistance(6.0f);
        SSGI->SetThickness(0.2f);
        SSGI->SetSurfaceBias(0.05f);
        SSGI->SetMaxSteps(16u);
        SSGI->SetRayCount(4u);
        SSGI->SetDepthSigma(64.0f);
        SSGI->SetNormalSigma(32.0f);
        SSGI->SetRadianceClamp(2.5f);
        SSGI->SetMaxPixelRadius(96.0f);
        SSGI->SetStepExponent(1.25f);
        SSGI->SetTemporalBlendFactor(0.08f);
        SSGI->SetDisocclusionThreshold(0.02f);
        SSGI->SetClampStrength(0.10f);
        SSGI->SetVelocityWeight(12.0f);
        SSGI->SetLowLumaBoost(0.08f);
        SSGI->SetTemporalDebugMode(0u);
        m_graphics->RegisterPass(RendererViewportID, std::move(SSGI));
        m_graphics->RegisterPass(RendererViewportID, std::make_unique<CompositePass>(std::move(SSGICompositePassProperties)));
    }
    ...
    if (m_settings.EnableSsr)
    {
        ...
        if (m_settings.EnableSsgi)
        {
            SSRPassProperties[AutoGeneratedPass::PropertyNames::PassInputResourceNameOverrides.data()] =
                ResourceNameMappings{{"Shading_Out", "Shading_SSGI"}};
        }
        ...
        if (m_settings.EnableSsgi)
        {
            SSRCompositePassProperties[AutoGeneratedPass::PropertyNames::PassInputResourceNameOverrides.data()] =
                ResourceNameMappings{{"Shading_Out", "Shading_SSGI"}};
        }
    ...
    if (m_settings.EnableHeightFog)
    {
        m_graphics->RegisterPass(RendererViewportID, std::make_unique<HeightFogPass>(std::move(HeightFogPassProperties)));
    }
    ...
    {
        auto TAA = std::make_unique<TAAPass>(std::move(TAAPassProperties));
        TAA->SetBlendFactor(0.06f);
        TAA->SetMotionBlendFactor(0.18f);
        TAA->SetClampStrength(0.10f);
        TAA->SetSharpen(0.0f);
        m_graphics->RegisterPass(RendererViewportID, std::move(TAA));
    }
    ...
    if (m_settings.EnableBloom)
    {
        ...
        if (m_settings.EnableTaa)
        {
            BloomPassProperties[AutoGeneratedPass::PropertyNames::PassInputResourceNameOverrides.data()] =
                ResourceNameMappings{{"AtmosComposite_Out", "TAA_Out"}};
        }
    ...
    m_graphics->ClearEnvironmentCapturePassGraph(RendererViewportID);

    auto CaptureGBufferPassProperties = PassProperties{
        {AutoGeneratedPass::PropertyNames::PassName.data(), "Environment Capture GBuffer Pass"},
        {AutoGeneratedPass::PropertyNames::MaterialsShadingModel.data(), "GBufferShadingModel"},
        {GBufferPass::PropertyNames::ShadersCullProgram.data(), "TriangleCulling"},
        {GBufferPass::PropertyNames::ShadersGenDrawIndirectProgram.data(), "GenDrawIndirect"},
        {GBufferPass::PropertyNames::ShadersReductionProgram.data(), "TriangleReduction"},
        {GBufferPass::PropertyNames::ShadersInstanceCullProgram.data(), "InstanceCulling"},
        {AutoGeneratedPass::PropertyNames::PassDepthConfig.data(),
         DepthConfig{.WriteDepth = true, .DepthTest = true, .WriteResourceName = "GBuffer_Depth"}}};

    auto CaptureShadingPassProperties = PassProperties{
        {AutoGeneratedPass::PropertyNames::PassName.data(), "Environment Capture Shading Pass"},
        {FullScreenPass::PropertyNames::MaterialsShadingModel.data(), "DeferredShadingShadingModel"},
        {FullScreenPass::PropertyNames::MaterialsModule.data(), "DefaultDeferredMaterial"},
        {AutoGeneratedPass::PropertyNames::PassDepthConfig.data(),
         DepthConfig{.WriteDepth = false, .SampleDepth = true, .DepthTest = false, .ReadResourceName = "GBuffer_Depth", .ReadLayout = EImageLayout::DepthStencilReadOnlyOptimal}}};

    auto CaptureHeightFogPassProperties = PassProperties{
        {AutoGeneratedPass::PropertyNames::PassName.data(), "Environment Capture Height Fog Pass"},
        {FullScreenPass::PropertyNames::MaterialsShadingModel.data(), "PostProcessShadingModel"},
        {FullScreenPass::PropertyNames::MaterialsModule.data(), "HeightFogMaterial"},
        {AutoGeneratedPass::PropertyNames::PassInputResourceNameOverrides.data(),
    ...
    auto CaptureAtmospherePassProperties = PassProperties{
        {AutoGeneratedPass::PropertyNames::PassName.data(), "Environment Capture Atmosphere Pass"},
        {FullScreenPass::PropertyNames::MaterialsShadingModel.data(), "PostProcessShadingModel"},
        {FullScreenPass::PropertyNames::MaterialsModule.data(), "AtmosphereMaterial"},
    };

    auto CaptureAtmosphereCompositePassProperties = PassProperties{
        {AutoGeneratedPass::PropertyNames::PassName.data(), "Environment Capture Atmosphere Composite Pass"},
        {FullScreenPass::PropertyNames::MaterialsShadingModel.data(), "PostProcessShadingModel"},
        {FullScreenPass::PropertyNames::MaterialsModule.data(), "AtmosCompositeMaterial"},
        {AutoGeneratedPass::PropertyNames::PassDepthConfig.data(),
         DepthConfig{.WriteDepth = false, .SampleDepth = true, .DepthTest = false, .ReadResourceName = "GBuffer_Depth", .ReadLayout = EImageLayout::DepthStencilReadOnlyOptimal}}};

    std::string EnvironmentCaptureFinalColorResourceName = "Shading_Out";

    ShadowPass::Descriptor CaptureShadowDescriptor{};
    CaptureShadowDescriptor.ShadowMapSize = 128u;
    CaptureShadowDescriptor.MaxDirectionalLights = 1u;
    CaptureShadowDescriptor.MaxCascadesPerDirectional = 1u;
    CaptureShadowDescriptor.MaxPointLights = 1u;
    CaptureShadowDescriptor.MaxSpotLights = 1u;

    auto CaptureShadow = std::make_unique<ShadowPass>(CaptureShadowDescriptor);
    CaptureShadow->SetLightManager(ActiveLightManager);
    m_graphics->RegisterEnvironmentCapturePass(RendererViewportID, std::move(CaptureShadow));
    m_graphics->RegisterEnvironmentCapturePass(RendererViewportID, std::make_unique<GBufferPass>(std::move(CaptureGBufferPassProperties)));
    {
        auto CaptureDeferredShading = std::make_unique<DeferredShadingPass>(std::move(CaptureShadingPassProperties));
        CaptureDeferredShading->SetLightManager(ActiveLightManager);
        m_graphics->RegisterEnvironmentCapturePass(RendererViewportID, std::move(CaptureDeferredShading));
    }

    if (m_settings.EnableHeightFog)
    {
        m_graphics->RegisterEnvironmentCapturePass(RendererViewportID, std::make_unique<HeightFogPass>(std::move(CaptureHeightFogPassProperties)));
        EnvironmentCaptureFinalColorResourceName = "HeightFog_Out";
    }
    ...
    if (m_settings.EnableAtmosphere)
    {
        if (m_settings.EnableHeightFog)
        {
            CaptureAtmosphereCompositePassProperties[AutoGeneratedPass::PropertyNames::PassInputResourceNameOverrides.data()] =
                ResourceNameMappings{{"Shading_SSR", "HeightFog_Out"}};
        }
        else
        {
            CaptureAtmosphereCompositePassProperties[AutoGeneratedPass::PropertyNames::PassInputResourceNameOverrides.data()] =
        ...
        auto CaptureAtmosphere = std::make_unique<AtmospherePass>(std::move(CaptureAtmospherePassProperties));
        CaptureAtmosphere->SetFeature(AtmospherePass::Feature::World, m_settings.AtmosphereWorldMode);
        CaptureAtmosphere->SetSunDirection(15, 0, 0);
        m_graphics->RegisterEnvironmentCapturePass(RendererViewportID, std::move(CaptureAtmosphere));
        m_graphics->RegisterEnvironmentCapturePass(RendererViewportID, std::make_unique<AtmosphereCompositePass>(std::move(CaptureAtmosphereCompositePassProperties)));
        EnvironmentCaptureFinalColorResourceName = "AtmosComposite_Out";
    }

    m_graphics->SetEnvironmentCaptureFinalColorResourceName(RendererViewportID, EnvironmentCaptureFinalColorResourceName);
