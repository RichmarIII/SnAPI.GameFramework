...
std::vector<fs::path> CollectRegistrationScanFiles(const fs::path& ProjectRoot, const fs::path& SeedSource)
{
    ...
    auto AddRootIfExists = [&Roots, &SeenRoots](const fs::path& Root) {
        if (!fs::exists(Root))
        {
            return;
        }
        ...
        if (SeenRoots.insert(NormalizedRoot.generic_string()).second)
        {
    ...
    {
        AddRootIfExists(ProjectRoot / "include");
        AddRootIfExists(ProjectRoot / "src");

        const fs::path ModulesRoot = ProjectRoot / "Modules";
        const fs::path NormalizedSeed = NormalizePath(SeedSource);
        if (fs::exists(ModulesRoot) && IsPathWithin(NormalizedSeed, ModulesRoot))
        {
            fs::path ModuleRoot = NormalizePath(ModulesRoot);
            fs::path Relative = fs::relative(NormalizedSeed, ModuleRoot);
            if (!Relative.empty() && Relative.begin() != Relative.end() && *Relative.begin() != ".")
            {
                ModuleRoot /= *Relative.begin();
                AddRootIfExists(ModuleRoot / "Public");
                AddRootIfExists(ModuleRoot / "Private");
            }
        }

        if (IsPathWithin(SeedSource, ProjectRoot / "tests"))
        {
            AddRootIfExists(ProjectRoot / "tests");
        }
    }
