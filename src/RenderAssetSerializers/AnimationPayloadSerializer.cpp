#include "RenderAssetSerializers/AnimationPayloadSerializer.h"

#include "AssetPipelineIds.h"
#include "RenderAssets/AnimationPayload.h"
#include "PayloadSerializerHelpers.h"

namespace SnAPI::GameFramework
{
namespace
{

class AnimationPayloadSerializer final : public ::SnAPI::AssetPipeline::IPayloadSerializer
{
public:
    ::SnAPI::AssetPipeline::TypeId GetTypeId() const override { return PayloadAnimation(); }
    const char* GetTypeName() const override { return kPayloadAnimationName; }
    uint32_t GetSchemaVersion() const override { return 1u; }

    void SerializeToBytes(const void* Object, std::vector<uint8_t>& OutBytes) const override
    {
        Detail::SerializePayloadObject<AnimationPayload, SerializeAnimationPayload>(Object, OutBytes);
    }

    bool DeserializeFromBytes(void* Object, const uint8_t* Bytes, std::size_t Size) const override
    {
        return Detail::DeserializePayloadObject<AnimationPayload, DeserializeAnimationPayload>(Object, Bytes, Size);
    }
};

} // namespace

std::unique_ptr<::SnAPI::AssetPipeline::IPayloadSerializer> CreateAnimationPayloadSerializer()
{
    return std::make_unique<AnimationPayloadSerializer>();
}

} // namespace SnAPI::GameFramework
