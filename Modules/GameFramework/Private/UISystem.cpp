...
void UISystem::PushInput(const SnAPI::UI::PointerEvent& EventValue)
{
    ...
    const SnAPI::UI::UIPoint ScreenPosition = [&]() {
        if (const auto* RootContext = FindContextLocked(m_rootContextId))
        {
            return RootContext->MapInputPointToScreenPoint(EventValue.Position);
        }
        return EventValue.Position;
    }();

    m_lastPointerPosition = ScreenPosition;
    m_hasLastPointerPosition = true;
    SnAPI::UI::PointerEvent ScreenEvent = EventValue;
    ScreenEvent.Position = ScreenPosition;
    ...
    else
    {
        ...
        ContextId TargetContext = FindDeepestPointerTargetLocked(m_rootContextId, ScreenPosition);
        if (TargetContext == 0)
        {
    ...
    for (const auto ContextIdValue : DispatchContexts)
    {
        if (auto* ContextValue = FindContextLocked(ContextIdValue))
        {
            ContextValue->PushInput(ScreenEvent);
        }
    }

    m_pointerLeftDown = ScreenEvent.LeftDown;
    m_pointerRightDown = ScreenEvent.RightDown;
    m_pointerMiddleDown = ScreenEvent.MiddleDown;
...
void UISystem::PushInput(const SnAPI::UI::WheelEvent& EventValue)
{
    ...
    m_lastPointerPosition = ScreenPosition;
    m_hasLastPointerPosition = true;
    SnAPI::UI::WheelEvent ScreenEvent = EventValue;
    ScreenEvent.Position = ScreenPosition;
    ...
    {
        const ContextId PreviousActiveContext = m_activeInputContext;
        TargetContext = FindDeepestPointerTargetLocked(m_rootContextId, ScreenPosition);
        if (TargetContext == 0)
        {
...
SnAPI::UI::UIPoint UISystem::MapInputPointToScreenPoint(const SnAPI::UI::UIPoint Point) const
{
    SNAPI_GF_PROFILE_FUNCTION("UI");
    GameLockGuard Lock(m_mutex);
    if (!m_initialized || m_rootContextId == 0)
    ...
    if (const auto* RootContext = FindContextLocked(m_rootContextId))
    {
        return RootContext->MapInputPointToScreenPoint(Point);
    }
    return Point;
}
